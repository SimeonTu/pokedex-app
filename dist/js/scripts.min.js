const colorThief=new ColorThief;let pokemonList=[],missingNo="img/missingNo.png",offset,gen,end,results,searchFlag=0,finalOffset,flag=0,loadBtn=$("#btn-load-pokemon"),loadingWrapper=$(`<div class="loading-wrapper">
                          ...
                          </div>`);const RGBToHSL=(e,o,t,n)=>{e/=255,o/=255,t/=255;let a=Math.max(e,o,t),s=a-Math.min(e,o,t),l=s?a===e?(o-t)/s:a===o?2+(t-e)/s:4+(e-o)/s:0,r=100*(s?a<=.5?s/(2*a-s):s/(2-(2*a-s)):0)+"%",d=100*(2*a-s)/2;return d+=parseInt(n),[60*l<0?60*l+360:60*l,r,d=d>100?"100%":d<0?"0%":d+"%"].toString()},intersectionObserver=new IntersectionObserver(async function(e){console.log(e),e[0].isIntersecting&&0==flag&&(console.log("flag within observer: "+flag),$("observer").remove(),console.log("SEEN!!"),console.log("NOT OBSERVING"),console.log("search flag within observer: "+searchFlag),1==searchFlag?loadDataFromSearch(offset,offset+16):loadData(offset+1,offset+16),setTimeout(()=>{$("#pokelist").append($("<observer></obeserver>")),intersectionObserver.observe($("observer")[0]),console.log("OBSERVING")},2200))}),delay=e=>new Promise(o=>setTimeout(o,e));function addListItem(e){flag=3;let o=$(`<div class="col col-xs-12 col-sm-6 col-md-4 col-lg-3 poke-item-col poke-item-col-${e.id}" data-bs-toggle="modal" data-bs-target="#pokeModal">
